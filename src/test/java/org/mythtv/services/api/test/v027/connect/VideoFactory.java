...
public class VideoFactory extends ResponseFactory {
	...
	public AbstractMythFakeHttpInputMessage create(URI url) {
		if (url.getPath().endsWith("/GetVideo")){
			return new org.mythtv.services.api.test.v026.connect.MythFakeHttpInputMessage("video/GetVideo.json");
		} else if (url.getPath().endsWith("/GetBluray")){
			return new org.mythtv.services.api.test.v026.connect.MythFakeHttpInputMessage("video/GetBluray.json");
		} else if (url.getPath().endsWith("/GetVideoByFileName")){
			return new org.mythtv.services.api.test.v026.connect.MythFakeHttpInputMessage("video/GetVideoByFileName.json");
		} else if (url.getPath().endsWith("/GetVideoList")){
			return new org.mythtv.services.api.test.v026.connect.MythFakeHttpInputMessage("video/GetVideoList.json");
